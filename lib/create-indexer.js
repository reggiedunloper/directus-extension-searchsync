...
function createIndexer(config, { logger, database, services, getSchema }) {
	...
	async function initCollectionIndexes() {
		if (config.singleIndexName) {
			try {
				await indexer.deleteItems(config.singleIndexName);
			} catch (error) {
				logger.warn(`Cannot drop collection "${config.singleIndexName}". ${getErrorMessage(error)}`);
				logger.debug(error);
			}
		}		
